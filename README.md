...
## Q10
>+ **1．编程计算由下列公式确定的S值，其中n是用户输入的正整数。S = 2/1 + 3/2 + 5/3 + 8/5 + 13/8 + 21/13 + …**
...
## Q14
>+ **编写传统的石头、剪刀、布的游戏程序，单局胜负规则是，剪刀胜布，布胜石头，石头胜剪刀。用户玩家和计算机对阵，采用7局4胜制（为了简单便于判定，设计算机的出拳方式是固定的）。最后输出胜负或平局信息。请编程模拟实现。**  
